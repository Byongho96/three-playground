...
export const animationGroup = ({
  ...
  gltfLoader.load(bodyGlb, (gltf) => {
    ...
    castObject(body)
    objects.push(body)
    animationGroup.add(body)

    const bodyMixer = new THREE.AnimationMixer(animationGroup)
    mixers.push(bodyMixer)

    animationClips['run'] = gltf.animations[0]
    scene.add(body)
...
function playAnimation(mixers: THREE.AnimationMixer[]) {
  mixers.forEach((mixer) => {
    mixer.clipAction(animationClips['run']).reset().fadeIn(0.5).play()
  })
}
